describe 'With correct authentication credentials' do
  ...
  context 'signing in' do
    it 'will return a json token' do
      post '/sign_in', session: { name: account.name, password: account.password }
  ...
  context 'signing out' do
    it 'will redirect' do
      get '/sign_out', nil, HTTP_AUTHORIZATION: ActionController::HttpAuthentication::Token.encode_credentials(account.token)
